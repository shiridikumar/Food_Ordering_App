        ...
        const loadpost= async()=>{
            await axios.post('http://localhost:4000/user/myorders',{crossdomain:true,email:details.email}).then(response=>{
                ...
                for(var i=0;i<response.data.length;i++){
                    ...
                    row.push(<Ordered canteen={response.data[i].shop_name} food={response.data[i].food} amount={response.data[i].cost} itemid={ids} quantity={response.data[i].quantity} status={response.data[i].status} time={response.data[i].Time}/>)
                    console.log(row);
